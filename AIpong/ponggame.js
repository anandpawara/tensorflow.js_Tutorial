...
model.add(tf.layers.dense({units:3,inputShape:[256]}));//return 1x3
...
//Custom code
//Moving computer players 4x according to neural output -1,0,1
Computer.prototype.ai_update = function(move = 0) {
  this.paddle.move(4 * move,0);
};
...
AI.prototype.save_data = function(player, computer, ball){
  ...
  if(this.previous_data == null){
    data = this.flip_table ? [width -computer.x,width - player.x,width - ball.x,height - ball.y]:
  ...
  if(this.flip_table){
    data_xs = [width - computer.x,width - player.x,width - ball.x];
    index = ((width - player.x) > this.previous_data[1]? 0:((width - player.x) == this.previous_data[1])?1:2);
  }else{
      data_xs = [player.x,computer,ball.x,ball.y];
